...
describe("sendCheckoutContinueEmail", () => {
  ...
  it("prefers PUBLIC_SITE_URL over the provided base URL", async () => {
    ...
    expect(body.TextBody).toContain("https://lan.example.test:3100/checkout/continue?session_id=cs_test");
  });
  ...
  it("falls back to the provided base URL when no env override exists", async () => {
    ...
    const body = JSON.parse(fetchMock.mock.calls[0][1]?.body as string);
    expect(body.TextBody).toContain("https://app.local.test:4000/checkout/continue?session_id=cs_test");
  });
});
