...
describe("computeEntitlementUsage", () => {
  it("computes remaining with lower bound at zero", () => {
    ...
    expect(usage.concepts).toEqual({ limit: 3, consumed: 5, remaining: 0 });
    expect(usage.revisions).toEqual({ limit: 2, consumed: 1, remaining: 1 });
  });

  it("falls back to package defaults when rows are missing", () => {
    const usage = computeEntitlementUsage([], "essential");
    expect(usage.concepts).toEqual({ limit: 2, consumed: 0, remaining: 2 });
    expect(usage.revisions).toEqual({ limit: 2, consumed: 0, remaining: 2 });
  });
});
