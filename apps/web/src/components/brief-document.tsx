      ...
      <header className="border-b border-neutral-200 pb-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="max-w-3xl">
            ...
            {subtitle ? <p className="mt-1.5 max-w-[72ch] text-sm leading-6 text-neutral-700">{subtitle}</p> : null}
            {meta ? <div className="mt-2 text-xs text-neutral-600">{meta}</div> : null}
          </div>
  ...
  return (
    <section className={`rounded-2xl border p-5 ${toneClass} ${className}`.trim()}>
      <div className="border-b border-neutral-200 pb-3">
        <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-neutral-600">Section</p>
        <h3 className="mt-1 text-base font-semibold text-neutral-900">{title}</h3>
        {description ? <p className="mt-1.5 max-w-[72ch] text-sm leading-6 text-neutral-700">{description}</p> : null}
      </div>
...
export function BriefField({ label, value, compact = false, className = "", valueClassName = "" }: BriefFieldProps) {
  return (
    <section className={`rounded-xl border border-neutral-200 bg-white ${compact ? "p-3" : "p-4"} ${className}`.trim()}>
      <h4 className="text-[11px] font-semibold uppercase tracking-[0.08em] text-neutral-600">{label}</h4>
      <p className={`mt-2.5 max-w-[72ch] whitespace-pre-wrap text-[15px] text-neutral-900 ${compact ? "leading-6" : "leading-7"} ${valueClassName}`.trim()}>{value || "—"}</p>
    </section>
  );
}
